...
def blast_primer(item, genomename):
    ...
    pR = p.append(SeqRecord(seq = Seq(item[1], IUPACAmbiguousDNA()), id=str(str(item[1])), name = "R", description = ""))
...
def primer_search(current_amp, global_parameters=global_parameters, filename="primerlist.tsv", method="dumb", tm=40):
    ...
    while bad == 1 and expand < expansion_limit:
        ...
        if bad == 0:
            product_nonoverlapping_guidecount = eating.base.count_non_overlapping_guides(eating.base.digest_target(current_amp.permissible_region[int(leftprimer_start):int(rightprimer_start)+int(rightprimer_length)]))
            with open(filename, "a") as primerlist:
